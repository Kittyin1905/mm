				...
				$("#records").append(
					$('<li>').attr("data-date", dataDate).append(
						...
						$('<em>').text(recordYear),
						 ...
						 $('<a>').attr("href", recordImageLarge).addClass("strip").attr("data-strip-caption",recordTitle).append($('<img>').addClass("watch").attr("src", recordImage)),
						
						$('<p>').text(recordDescription)
					)
				);
				
				Tipped.create(".watch", "Click Picture to view!", { position: 'bottom'});
			}
		}
